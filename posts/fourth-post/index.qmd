...
description: "Functions from Lab 6"
author:
  - name: Jose Garcia
date: 2025-05-12
categories: [R]
...
remove_outliers <- function(df, ..., sd_thresh = 3) {
  ...
  cols <- enquos(...)
  col_names <- map_chr(cols, as_name)
  ...
  df |>
    mutate(across(all_of(col_names), ~ 
                    abs((. - mean(., na.rm = TRUE)) / sd(., na.rm = TRUE)))) |>
    filter(if_all(all_of(col_names), ~ . <= sd_thresh | is.na(.)))
}
